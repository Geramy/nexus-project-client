...
class BuildService {
  ...
  Future<StartedRun> startDockerBuild({
    ...
    final runPk = await db.createCiRun(
    ...
    final jobPk = await db.createCiJob(
    ...
    final stepPk = await db.createCiStep(
  ...
  Future<void> _runDockerBuild({
    ...
    try {
      ...
      final outcome = await _backendFor(
        backend,
      ).execute(req, log: _logSink(stepPks), cancel: cancel);
      await _finalize(
        runPk,
        jobPks,
        stepPks,
  ...
  Future<StartedRun> startWorkflowRun({
    ...
    final plan = workflowParser.parse(
      yamlText,
      fileName: _basename(workflowPath),
    );
      ...
      for (var s = 0; s < job.steps.length; s++) {
        ...
        final stepPk = await db.createCiStep(
          CiStepsCompanion.insert(
            ...
            orderIndex: Value(s),
            command: Value(
              step.run ?? (step.uses != null ? 'uses: ${step.uses}' : null),
            ),
          ),
        );
        steps.add(stepPk);
  ...
  Future<void> _runWorkflow({
    ...
    try {
      await db.patchCiRun(
        runPk,
        CiRunsCompanion(status: const Value('running'), startedAt: Value(now)),
      );
          ...
          if (j < stepPks.length && s < stepPks[j].length) {
            db.patchCiStep(
              ...
              CiStepsCompanion(
                ...
                startedAt: status == CiStatus.running
                    ? Value(DateTime.now())
                    : const Value.absent(),
  ...
  CiLogSink _logSink(List<List<int>> stepPks) {
    int? resolve(int? j, int? s) {
      ...
      if (stepPks.isNotEmpty && stepPks.first.isNotEmpty)
        return stepPks.first.first;
      return null;
    }
  ...
  Future<void> _markRunning(int runPk, int jobPk, int stepPk) async {
    ...
    await db.patchCiJob(
      jobPk,
      CiJobsCompanion(status: const Value('running'), startedAt: Value(now)),
    );
    await db.patchCiStep(
      stepPk,
      CiStepsCompanion(status: const Value('running'), startedAt: Value(now)),
    );
  }
  ...
  Future<void> _finalize(
    ...
    for (var j = 0; j < outcome.jobs.length && j < jobPks.length; j++) {
      ...
      for (
        var s = 0;
        s < job.steps.length && j < stepPks.length && s < stepPks[j].length;
        s++
      ) {
        final step = job.steps[s];
        await db.patchCiStep(
    ...
    if (taskPk != null) {
      await db.recordTaskBuildResult(
        taskPk,
        passed: outcome.status == CiStatus.success,
      );
    }
  }
  ...
  Future<void> _failAll(
    ...
    for (var j = 0; j < jobPks.length; j++) {
      await db.patchCiJob(
        jobPks[j],
        CiJobsCompanion(status: const Value('failed'), completedAt: Value(now)),
      );
