...
import 'package:nexus_projects_client/infrastructure/database/nexus_database.dart' show TasksCompanion;
...
class _OverviewTabState extends ConsumerState<OverviewTab> {
  ...
  Future<void> _load() async {
    final task = await ref.read(nexusDatabaseProvider).getTaskById(widget.taskId);
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(nexusDatabaseProvider).patchTask(
            widget.taskId,
            TasksCompanion(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(ctx, true), child: Text('Delete', style: TextStyle(color: ctx.nx.danger))),
        ],
      ),
    );
    if (ok != true) return;
  ...
  Widget _provenance() {
    if (_planPath == null && _chatSessionFk == null) return const SizedBox.shrink();
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_planPath != null)
            Row(children: [
              ...
              Flexible(child: Text(_basename(_planPath!), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
              TextButton(
                style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: AppSpacing.sm), minimumSize: Size.zero),
                onPressed: () {
                  ref.read(openPlanNotifierProvider.notifier).open(_planPath);
          ...
          if (_chatSessionFk != null)
            Padding(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 6),
                Text('Created in coordinator conversation #$_chatSessionFk', style: const TextStyle(fontSize: 12)),
              ]),
            ),
        ]),
      ),
    ...
    return Scrollbar(
      ...
      child: Column(
        ...
        children: [
          ...
          Gap.md,
          Row(children: [
            Expanded(child: _dateField('Start date', _startDate, () => _pickDate(isStart: true), () => setState(() => _startDate = null))),
            const SizedBox(width: AppSpacing.md),
            Expanded(child: _dateField('Due date', _dueDate, () => _pickDate(isStart: false), () => setState(() => _dueDate = null))),
          ]),
          Gap.md,
          personasAsync.when(
            data: (personas) {
              final items = <DropdownMenuItem<int?>>[
                ...
                for (final p in personas) DropdownMenuItem(value: p.agent_pk, child: Text(p.name, overflow: TextOverflow.ellipsis)),
              ];
              final valid = _agentId != null && personas.any((p) => p.agent_pk == _agentId) ? _agentId : null;
              return DropdownButtonFormField<int?>(
                initialValue: valid,
                isExpanded: true,
                decoration: const InputDecoration(labelText: 'Assigned agent', border: OutlineInputBorder(), isDense: true),
                items: items,
                onChanged: (v) => setState(() => _agentId = v),
              );
            },
            loading: () => const LinearProgressIndicator(),
            error: (e, _) => Text('Agents error: $e', style: const TextStyle(fontSize: 12)),
          ),
          _provenance(),
          ...
          Wrap(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: Text('Delete Task', style: TextStyle(color: context.nx.danger)),
              ),
            ],
          ),
        ],
  ...
  Widget _dateField(String label, DateTime? value, VoidCallback onPick, VoidCallback onClear) {
    return InputDecorator(
      ...
      child: Row(
        children: [
          Expanded(child: Text(_fmt(value), style: const TextStyle(fontSize: 13))),
          if (value != null)
            InkWell(onTap: onClear, child: const Icon(Icons.clear, size: 16)),
          const SizedBox(width: 4),
          InkWell(onTap: onPick, child: const Icon(Icons.calendar_today, size: 16)),
        ],
      ),
    );
  }
