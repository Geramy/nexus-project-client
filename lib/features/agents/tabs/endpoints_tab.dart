...
class EndpointsTab extends ConsumerWidget {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Text('Inference Servers', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
              const SizedBox(width: 8),
              Text('(Client: $currentClientId)', style: const TextStyle(fontSize: 11, color: Colors.grey)),
              const Spacer(),
              FilledButton.icon(
                onPressed: () => _showAddServerDialog(context, ref, currentClientId),
          ...
          Expanded(
            child: serversAsync.when(
              data: (servers) {
                if (servers.isEmpty) {
                  return const Center(child: Text('No inference servers configured for this client yet.'));
                }
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        onTap: () => _editServerViaDialog(context, ref, ui, currentClientId),
                      ),
                    );
                  },
                );
  ...
  Future<void> _showAddServerDialog(BuildContext context, WidgetRef ref, int clientId) async {
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Server Name')),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    DropdownMenuItem(value: 'lemonade', child: Text('Lemonade')),
                  ...
                  onChanged: (v) {
                    if (v != null) {
                      setState(() {
                        selectedType = v;
                        nameCtrl.text = suggestName(v); // live-update suggestion based on chosen type
                      });
                    }
                  },
                ),
                ...
                Slider(value: concurrency.toDouble(), min: 1, max: 32, divisions: 31, onChanged: (v) => setState(() => concurrency = v.round())),
                Text('Max Agents: $maxAgents'),
                Slider(value: maxAgents.toDouble(), min: 1, max: 50, divisions: 49, onChanged: (v) => setState(() => maxAgents = v.round())),
              ],
            ),
          ),
          actions: [
  ...
  Future<void> _updateServerInDb(WidgetRef ref, ui_model.InferenceServer updated) async {
    final db = ref.read(nexusDatabaseProvider);

    final availableModelsJson = jsonEncode(updated.availableModels);
    final capabilitiesJson = jsonEncode(updated.capabilities);

    await (db.update(db.inferenceServers)..where((s) => s.server_pk.equals(int.parse(updated.id)))).write(
            ...
            children: otherClients.map((client) {
              return ListTile(
                ...
                onTap: () async {
                  ...
                  Navigator.pop(ctx);
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Cloned server to "${client.name}"')),
                  );
                },
              );
            }).toList(),
