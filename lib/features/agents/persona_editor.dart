...
class PersonaEditor extends ConsumerStatefulWidget {
  ...
  const PersonaEditor({super.key, required this.personaName, this.initialModel = 'Claude 4', this.personaId});
...
class _PersonaEditorState extends ConsumerState<PersonaEditor> {
  ...
  final Map<String, String?> _mods = {'tts': null, 'stt': null, 'imageGen': null, 'vision': null, 'llm': null};
  ...
  void initState() {
    ...
    systemPromptCtrl = TextEditingController(
        text: 'Senior backend engineer focused on security, clean architecture, and production-grade code.');
  ...
  Future<void> _loadPersona() async {
    ...
    final row = await ref.read(nexusDatabaseProvider).resolveAgentPersona(widget.personaId!);
    if (mounted && row != null) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final serversAsync = ref.watch(inferenceServersForClientProvider(currentClientId));
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Gap.md,
        TextFormField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Persona Name', border: OutlineInputBorder())),
        Gap.md,
        DropdownButtonFormField<AgentRole>(
          ...
          decoration: const InputDecoration(
            ...
            helperText: 'The orchestrator uses this role to pick the agent for a task and to apply its default skills, tools, and system prompt.',
        ...
        Gap.sm,
        Row(children: [
          Expanded(child: TextFormField(initialValue: '35', decoration: const InputDecoration(labelText: 'Max USD per Task', border: OutlineInputBorder()))),
          Gap.md,
          Expanded(child: TextFormField(initialValue: '120', decoration: const InputDecoration(labelText: 'Max USD per Hour', border: OutlineInputBorder()))),
        ]),
      ]),
    );
  }
  ...
  Widget _aiProviderSection(AsyncValue<List<InferenceServer>> servers) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      servers.when(
        data: (srvs) {
          if (srvs.isEmpty) return const NexusCard(padding: EdgeInsets.all(AppSpacing.md), child: Text('No AI Providers configured. Go to the AI Providers page.'));
          final items = <DropdownMenuItem<int?>>[const DropdownMenuItem(value: null, child: Text('Use client / project default'))];
          for (final s in srvs) {
            items.add(DropdownMenuItem(value: s.server_pk, child: Text('${s.providerType} • ${s.name}', overflow: TextOverflow.ellipsis)));
          }
          final valid = selectedAiProviderId != null && srvs.any((s) => s.server_pk == selectedAiProviderId) ? selectedAiProviderId : null;
          return DropdownButtonFormField<int?>(
            initialValue: valid,
            decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'AI Provider for this Persona'),
            isExpanded: true, items: items,
            onChanged: (v) => setState(() { selectedAiProviderId = v; }),
          );
        },
  ...
  Widget _modelRoutingSection() {
    ...
    if (!hasProvider) {
      return NexusCard(
        child: Text('Select an AI Provider above to see available models.',
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (omniCollectionModel != null && omniCollectionModel!.isNotEmpty) ...[
        ...
        _omniInfoCard(resolvePersonaModels(
          omniCollectionModel: omniCollectionModel,
          models: entry?.models ?? const <ApiModelInfo>[],
        )),
      ] else ...[
        Gap.md,
        Text('Individual Modality Models (all optional)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: context.nx.textMuted)),
        _modPicker('TTS — Text to Voice', Icons.record_voice_over_rounded, 'tts', individualModels, isLoading),
        Gap.sm,
        _modPicker('STT — Voice to Text', Icons.mic_none_rounded, 'stt', individualModels, isLoading),
        Gap.sm,
        _modPicker('Image Generation', Icons.image_outlined, 'imageGen', individualModels, isLoading),
        Gap.sm,
        _modPicker('Vision — Image Reading', Icons.visibility_rounded, 'vision', individualModels, isLoading),
        Gap.sm,
        _modPicker('LLM — Text Generation (Primary)', Icons.auto_awesome_mosaic, 'llm', individualModels, isLoading),
      ],
      Gap.md,
      _voiceSelector(),
    ]);
  ...
  Widget _voiceSelector() {
    final cur = (selectedVoice != null && kKokoroVoices.any((v) => v.id == selectedVoice)) ? selectedVoice : null;
    final items = <DropdownMenuItem<String?>>[
      DropdownMenuItem(value: null, child: Text('Default voice (${ttsVoiceLabel(kDefaultTtsVoice)})', overflow: TextOverflow.ellipsis)),
      for (final v in kKokoroVoices)
        DropdownMenuItem(value: v.id, child: Text(v.label, overflow: TextOverflow.ellipsis)),
    ];
    return NexusCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('The speaking voice for spoken replies (Kokoro TTS).',
              style: TextStyle(fontSize: 12, color: context.nx.textMuted)),
          const SizedBox(height: AppSpacing.sm),
          DropdownButtonFormField<String?>(
            initialValue: cur,
            isExpanded: true,
            decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'TTS Voice', isDense: true),
            items: items,
  ...
  Widget _omniSelector(List<ApiModelInfo> omniModels, bool isLoading) {
    final content = [
      Row(children: [
        ...
        const SizedBox(width: AppSpacing.sm),
        const Text('Omni Collection', style: TextStyle(fontWeight: FontWeight.w600)),
      ]),
      const SizedBox(height: AppSpacing.xs),
      Text('Select an Omni model to auto-configure all modalities using its default component models and tool routing.',
          style: TextStyle(fontSize: 12, color: context.nx.textMuted)),
      const SizedBox(height: AppSpacing.sm),
      if (isLoading)
  ...
  Widget _buildOmniDropdown(List<ApiModelInfo> omniModels) {
    ...
    if (items.length <= 1) return Padding(
      padding: const EdgeInsets.all(AppSpacing.sm),
      child: Text('No Omni Collection models available on this server. Use individual model selection below.',
          style: TextStyle(fontSize: 12, color: context.nx.textMuted)),
    );
  ...
  Widget _omniInfoCard(ResolvedModalityModels resolved) {
    ...
    return NexusCard(
      ...
      Row(children: [
        ...
        const Text('Component models from Omni Collection', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
      ]),
      ...rows.entries.map((e) => Padding(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: AppSpacing.sm),
              Text('${_modLabel(e.key)}:', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
              const SizedBox(width: AppSpacing.xs),
              Expanded(child: Text(e.value ?? '—',
                  style: TextStyle(fontSize: 12, color: nx.textFaint), overflow: TextOverflow.ellipsis)),
            ]),
          )),
    ]));
  }

  Widget _modPicker(String label, IconData icon, String key, List<ApiModelInfo> models, bool isLoading) {
    return NexusCard(
      ...
      child: Row(children: [
        ...
        Flexible(
          flex: 2,
          child: isLoading ? const SizedBox.shrink() : _buildModDropdown(models, key),
        ),
      ]),
    );
  }
  ...
  Widget _capabilityMatrix() {
    return NexusCard(
      ...
      child: Column(
        ...
        children: [
          Text('Controls which tools this agent may call. "Ask" prompts you to approve each call; "Deny" blocks it.',
              style: TextStyle(fontSize: 12, color: context.nx.textMuted)),
          const SizedBox(height: AppSpacing.xs),
          Row(children: [
            TextButton(onPressed: () => _setAllPerms(ToolPerm.grant), child: const Text('Grant all')),
            TextButton(onPressed: () => _setAllPerms(ToolPerm.ask), child: const Text('Ask all')),
            TextButton(onPressed: () => _setAllPerms(ToolPerm.deny), child: const Text('Deny all')),
          ]),
          for (final category in kToolCategories) ...[
            Padding(
              padding: const EdgeInsets.only(top: AppSpacing.sm, bottom: 2),
              child: Text(category, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
            ),
            ...kCoordinatorToolSpecs.where((t) => t.category == category).map((t) => Padding(
                  padding: const EdgeInsets.symmetric(vertical: 2),
                  child: Row(
                    children: [
                      Expanded(
                          ...
                          children: [
                            ...
                            if (t.destructive)
                              Padding(
                                ...
                                child: Icon(Icons.warning_amber_rounded, size: 13, color: context.nx.warning),
                              ),
                          ],
                        ),
                      ),
  ...
  Widget _permChip(String tool) {
    ...
    final label = cur == ToolPerm.grant ? 'Grant' : (cur == ToolPerm.ask ? 'Ask' : 'Deny');
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final cache = ref.read(aiServersCacheProvider);
      final entry = selectedAiProviderId != null ? cache[selectedAiProviderId!] : null;
      ...
      final updated = await (db.update(db.agentPersonas)
            ..where((p) => p.agent_pk.equals(widget.personaId!)))
          .write(
        AgentPersonasCompanion(
          name: Value(nameCtrl.text.trim()),
          title: Value(selectedRole?.key),
          description: Value(systemPromptCtrl.text.trim().isEmpty ? null : systemPromptCtrl.text.trim()),
      ...
      if (updated > 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Warning: no rows updated — persona may not exist.'), backgroundColor: Colors.orange),
        );
        return;
      }
  ...
  IconData _modIcon(String k) =>
      {'tts': Icons.record_voice_over_rounded, 'stt': Icons.mic_none_rounded, 'imageGen': Icons.image_outlined, 'vision': Icons.visibility_rounded, 'llm': Icons.auto_awesome_mosaic}[k]!;
  String _modLabel(String k) =>
      {'tts': 'TTS', 'stt': 'STT', 'imageGen': 'Image Gen', 'vision': 'Vision', 'llm': 'LLM'}[k]!;
}